...
- **Zero runtime risk** — Pure Markdown skill. The entire repository contains only `.md` files — no hooks, no shell scripts, no executable code, no `package.json`, no build step. Nothing runs on install or invocation beyond Copilot's APM skill loader.

## Installation

This skill ships with the APM package (ported from Everything Claude Code). No separate installation is needed when APM is installed.
...
If you are vendoring only this skill outside the full APM install, copy the reviewed file into `.github/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed ECC commit rather than running `git pull`.

## Requirements

- GitHub Copilot (load the blueprint skill and ask Copilot to plan using the blueprint pattern)
